...
exports.christmasName = function (message, logger) {
    ...
    message.reply(`Your new christmas name is ${message.member.displayName}!`);
}
...
exports.secretSanta = function (message, logger) {
    ...
    if (participants == null || !Array.isArray(participants) || participants.length < 1) {
        message.reply('You have to specify users who is participating in the secret santa by "mentioning" them. ( !secretSanta @firstPerson @secondPerson )');
        return;
    } else if (participants.length == 1) {
        message.reply("You can't have a secret santa with yourself. Mention more users.");
        return;
    } else if (isAnyUserBots(participants)) {
        message.reply("Bots can't participate in a secret santa. Mention users only.");
        return;
    }
    ...
    try{
        ...
        logger.error(`Validation failed with the following error: ${err}`);
        return;
    }
    ...
    for (let i = 0; i < participants.length; i++) {
        ...
        newDMChannel.then((value) => {
            ...
            logger.error(`Failed to create dm channel with user ${participants[i].tag} on textChannel ${message.channel.name}`);
        });
    }
}
...
function validateRules (a, b, logger) {
    ...
    if (rules != null) {
        ...
        if (validation) {
            if (rules.hasOwnProperty('pairs')) {
                ...
                if ((Array.isArray(a) && 2 == a.length) || (Array.isArray(b) && 2 == b.length)) {
                    logger.info('Attempted to use pair rules on arrays of size 2. (infinite loop)');
                    return true;
                }
        ...
        logger.info(`No rules specified. Everything checks out.`)
    }
...
function includeProtocol(a, b, pair) {
    if (Array.isArray(a) && Array.isArray(b) && 0 < a.length && 0 < b.length) {
        if (a[0] instanceof Discord.User && b[0] instanceof Discord.User) {
            ...
            if (!pairIncluded) {
                logger.info('Caught a rule violation!');
                return false;
            }
        } else {
...
exports.sendPepperkake = function (message, logger) {
    ...
    newDMChannel.then((value) => {
        ...
        logger.error(`Failed to create dm channel with user ${mentioned.tag} on textChannel ${message.channel.name}`);
    });
}
